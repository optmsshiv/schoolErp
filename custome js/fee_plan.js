    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === 'success' && response.data.length > 0) {
                ...
                // Assuming month_name is a comma-separated string from the backend
                const monthNames = feePlan.month_name.split(','); // This creates an array from the comma-separated string

                // Use Swal to display the editing form
                Swal.fire({
                    title: 'Edit Fee Plan',
                    html: `
                        <label>Id</label>
