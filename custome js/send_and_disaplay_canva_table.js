document.addEventListener("DOMContentLoaded", function () {
  ...
  const fetchFeeHeads = async (retryCount = 3, delayMs = 1000) => {
    ...
    try {
      ...
      populateFeeTypeDropdown(result.data);
    } catch (error) {
  ...
  const populateFeeTypeDropdown = (feeHeads) => {
  ...
  const addRowToTable = ({ feeMonth, feeType, feeAmount }) => {
    ...
    deleteButton.addEventListener("click", () => handleDeleteRow(newRow, feeAmount));
  ...
  // Update total amount
  const updateTotalAmount = (amountChange = 0) => {
    totalAmount += amountChange;
    payableAmountInput.value = totalAmount.toFixed(2);
  };

  // Handle Delete Fee Row
  const handleDeleteRow = (row, feeAmount) => {
    Swal.fire({
      title: "Are you sure?",
