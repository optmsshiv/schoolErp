$(function () {
  ...
  // Function to fetch student data with optional filters
  function fetchStudents(searchTerm = '', selectedClass = '') {
      $.ajax({
  ...
  function updatePageNumbers() {
      ...
      pageNumbers.find('button').on('click', function () {
          ...
          if (selectedPage !== currentPage) {
              ...
              fetchStudents($('#search-bar').val(), $('#class-select').val());
          }
      });
  }
  ...
  $('#search-bar').on('input', function () {
      currentPage = 1;
      fetchStudents($(this).val(), $('#class-select').val());
  });

  // Event listener for class selection
  $('#class-select').on('change', function () {
      currentPage = 1;
      fetchStudents($('#search-bar').val(), $(this).val());
  });
  ...
  $('#select-all').on('change', function () {
      ...
      $(this).toggleClass('mixed', this.checked);
  });
